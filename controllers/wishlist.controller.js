...
const getWishlistItemsHandler = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "Couldn't get wishlost items.Pleas try again later",
    });
  }
};
  ...
  try {
    ...
    if (!wishlist.find((wishlistItem) => wishlistItem.id === productId)) {
      return res.status(400).json({
        message: "Could not find product in wishlist",
      });
    }

    wishlist = wishlist.filter((wishlistItem) => wishlist.id !== wishlist);
    ...
    return res.status(500).json({
      message: "Couldn't delete item from wishlist.Plese try again later",
    });
  }
};
module.exports = {
